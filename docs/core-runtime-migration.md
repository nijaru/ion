...
- **joined runs against a worker-owned turn.** A joined worker now owns its own turn, so the creator cannot simply wait on it: the join needs a dependency that carries the final result back.
