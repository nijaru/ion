...
pub(crate) fn insert(
    ...
    connection.execute(
        "INSERT INTO conversations (id, parent_id, parent_at, owner_task, foreground_turn)
         VALUES (?1, ?2, ?3, ?4, ?5)",
        params![
            conversation.id.get(),
            conversation
...
pub(crate) fn load(connection: &Connection) -> Result<Vec<Conversation>, StoreError> {
    let mut statement = connection.prepare(
        "SELECT id, parent_id, parent_at, owner_task, foreground_turn
    ...
    for row in rows {
        let (id, parent_id, parent_at, owner_task, foreground_turn) = row?;
