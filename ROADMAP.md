...
| Storage topology/engine | Fresh per-session SQLite store implemented (schema v1, WAL + `synchronous = FULL`, commit-cursor fencing, open-time reconstruction); physical-topology/index/process-death evidence open | `crates/ion-core/src/store/sqlite/`; `crates/ion-core/tests/k4_sqlite.rs` |
