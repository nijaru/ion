...
Still open, in the order they block each other: **joined runs** (needs a dependency carrying the worker's final result; the initial generation settling is not that barrier), **worker-local turn scope** (a worker holds no foreground turn, so a follow-up admitted to a running worker could start a second generation beside the first), the command surface for send/follow-up, inspect and wait, interruption scoped to one worker run, reuse, and nested ownership limits.
