...
Input submission (`Session::submit_input` / `TaskDriver::submit_input`) admits the input and opens the turn that answers it in one commit, binding the input to the turn root (`Queued -> Assigned(task)`); the generation consumes it into the entry carrying its content (`Assigned -> Consumed(entry)`) in its settlement plan. Exact request-key replay is unchanged and no second turn is opened on replay; a submission whose key names an input that was merely admitted without a turn is refused rather than silently accepted.

Still open for this slice: a production provider catalogue, context/output bounds (P2), idle scheduling while a turn waits for the next input, a per-tool reconcile/adopt seam for tools that must query an external identity before retrying, and a bounded rule for cleanup created by cleanup (a successor born cancelled during abort cleanup still needs an explicit drive).
