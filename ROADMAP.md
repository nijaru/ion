...
Before SQL, bounded residency and the persistence interface are now decided rather than open. Residency uses typed indexed reads plus a small transaction overlay so no commit costs O(history) and checkpointing never hydrates unrelated historical payloads; the current full-`SessionState` clone per transaction is a K2 proof shortcut to remove in this stage. The decision and its invariants are recorded in `docs/core-runtime-migration.md`. Restricted task finalization and foreground-turn membership are implemented. The remaining interface work is the typed task authoring adapter and planned owned-conversation/scratch finalization.
