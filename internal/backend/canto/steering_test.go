...
func TestBackendFollowUpUsesCantoQueue(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	provider := &blockingFirstStreamProvider{
		FauxProvider: llm.NewFauxProvider(
			"local-api",
			llm.FauxStep{Content: "first answer"},
			llm.FauxStep{Content: "after follow-up"},
		),
		entered: make(chan struct{}),
		release: make(chan struct{}),
	}
	...
	queued, ok := b.Session().(ionsession.QueuedInputSession)
	if !ok {
		t.Fatal("canto session does not implement queued input")
	}
	result, err := queued.FollowUpTurn(ctx, "after this")
	if err != nil {
		t.Fatalf("follow up turn: %v", err)
	}
	if result.Outcome != ionsession.QueuedInputAccepted {
		t.Fatalf("follow-up outcome = %q, want accepted", result.Outcome)
	}
	close(provider.release)
	...
	if requestHasMessage(calls[0].Messages, llm.RoleUser, "after this") {
		t.Fatalf("first provider request unexpectedly contains follow-up: %#v", calls[0].Messages)
	}
	if !requestHasMessage(calls[1].Messages, llm.RoleUser, "after this") {
		t.Fatalf("second provider request missing follow-up: %#v", calls[1].Messages)
	}
}
