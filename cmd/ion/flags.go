...
func registerCLIFlags(fs *flag.FlagSet) cliFlags {
	return cliFlags{
		...
		trustProjectFlag: fs.Bool(
			"trust",
			false,
			"Trust project-local Ion resources for this directory",
		),
...
func (f cliFlags) trustProjectRequested() bool {
	return *f.trustProjectFlag
}
