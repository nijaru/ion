...
| C3 | Runtime queue and runner lifecycle | `../canto/runtime/runner.go`, `coordinator.go`, `coordinator_exec.go`, `options.go`, `bootstrap.go`, `hitl.go` | in_review | Queue wait vs execution context root cause fixed in Canto `595380a`. Runner/coordinator execution split was read; wait timeout and execution timeout are separate, queued wait failures append durable `TurnCompleted`, and lease ack/nack uses non-canceling contexts. | Finish `bootstrap.go`/`hitl.go`, then mark reviewed or patch. |
