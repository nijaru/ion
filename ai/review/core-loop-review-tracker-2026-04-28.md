...
The native baseline is single-path; deferred surfaces must be rejected at their owning boundaries rather than through a global mode.
...
| Order | Area | Files | Status | Evidence To Date | Next Check |
| --- | --- | --- | --- | --- | --- |
| I1 | Native baseline enforcement | provider/backend/app startup and deferred-surface owners | reviewed | The global stabilization flag is removed. ACP providers, telemetry startup, advanced slash commands, memory/subagent/MCP/reflexion processors, policy config, escalation config, and agent-visible compact tooling are out of the native baseline. Manual `/compact` plus proactive/overflow compaction remain active as reliability work. `@file` prompt expansion stays out because it silently mutates user prompts during the core-loop audit. | Keep new call sites out of the native baseline unless they are explicitly table-stakes for native loop reliability. |
| I2 | CLI startup, resume, continue, and print lifecycle | `cmd/ion/main.go`, `startup.go`, `selection.go`, `print_mode.go`, `mode.go`, `escalation.go`, `live_smoke_test.go` | reviewed | Startup/resume/print paths have tests for lazy session materialization, bare `--resume`, invalid config resume, print prompts/stdin/JSON/approval/timeout, submit failures, backend session errors, early stream close, empty assistant completion, and resumed marker order. External policy config and escalation config are out of the native baseline. OpenRouter Minimax live tool/resume/follow-up smoke passed after A4. | Keep future CLI work scoped to table-stakes automation gaps; do not reopen ACP/subscription CLI behavior before native parity is stable. |
| I3 | Backend contract and Canto event translation | `internal/backend/backend.go`, `unconfigured.go`, `internal/backend/canto/backend.go`, `compaction.go`, `processors.go`, `prompt.go` | patched | Event translation has focused tests for active-turn clearing, canceled terminal suppression, tool IDs/errors/output deltas, provider error recovery, retry status, proactive/overflow compaction, and valid resumed tool history. Direct MCP registration has been removed from the host session boundary while MCP remains deferred. | Continue reviewing SendStream synchronous-error handling and display-only write boundaries in I4 before marking reviewed. |
...
| I9 | Non-native/deferred packages | `internal/backend/acp/*.go`, `internal/privacy/*.go`, `internal/subagents/*.go`, `internal/telemetry/*.go`, workspace rollback/checkpoint code | reviewed | ACP backend construction is rejected through provider resolution. Telemetry startup is skipped. MCP, memory, subagent, and rewind commands are hidden/blocked or rejected at the catalog boundary, with no hidden TUI implementation for `/mcp` or `/rewind`. Manual `/compact` remains active as a host context-survival command. Privacy redaction remains a display-only transform for approval/tool text, and checkpoints remain active only as write-tool rollback metadata. | Keep these isolated; do not expand ACP/privacy/subagent/checkpoint UX before the native loop gate is closed. |
...
| Finding | Action |
| --- | --- |
| ACP providers were still reachable from normal provider resolution. | Reject ACP backend creation at provider resolution. ACP remains T4/secondary bridge work. |
...
| Telemetry setup ran during process startup. | Keep telemetry initialization out of the native baseline so T1 startup cannot fail on observability config/network issues. |
| `@file` prompt expansion was active in the native request processor stack. | Keep it out of the native baseline; the read tool is the explicit core path for file content. |
| External policy and escalation config loaded on startup. | Keep both out of the native baseline; optional config files cannot block the minimal loop. |
| MCP registration could still be invoked directly through the session interface. | Add a backend-level deferred-feature guard so tool-surface mutation is blocked below the slash-command layer. |
