...
- `tk-zxgq` queue UX first slice keeps the core loop stable: busy-turn text remains next-turn queued, but the TUI now shows queued text above the composer/progress shell and `Ctrl+G` pulls queued turns back into the composer before they are sent. True active-turn steering is tracked as `tk-8jz2` and must wait for an explicit backend contract.
