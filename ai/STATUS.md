...
- continue the core-loop code review with command/session lifecycle and app/backend handoff races now that Canto queue semantics, Ion's single-active-turn adapter guard, terminal active-state clearing, display-only replay ordering, single-writer storage boundaries, and TUI/print shutdown closing are covered.
