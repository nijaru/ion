# AI Context Index

## Root

- [STATUS](STATUS.md) — current phase, active task, latest evidence, and blockers.
- [PLAN](PLAN.md) — active roadmap and next implementation sequence.
- [DESIGN](DESIGN.md) — current Ion/Canto architecture and product boundaries.
- [DECISIONS](DECISIONS.md) — distilled principles plus recent decision log.

## Canonical Specs

- [ACP Integration](specs/acp-integration.md) — ACP bridge protocol and gaps.
- [Evals And Regression Gates](specs/evals-and-regression-gates.md) — test tiers and smoke gates.
- [Instructions And Skills](specs/instructions-and-skills.md) — instruction layering and skill boundaries.
- [Model Catalog Strategy](specs/model-catalog-strategy.md) — provider/model metadata flow.
- [Provider Support Expansion](specs/provider-support-expansion.md) — provider roadmap.
- [Security Policy](specs/security-policy.md) — approval, executor, environment, and secret policy.
- [Status And Config](specs/status-and-config.md) — config/state/status-line source of truth.
- [Subagent Personas](specs/subagent-personas-and-routing.md) — deferred subagent persona model.
- [Subscription Providers](specs/subscription-providers.md) — secondary subscription-provider path.
- [Swarm Mode](specs/swarm-mode-and-inline-subagents.md) — deferred operator-view model.
- [System Prompt](specs/system-prompt.md) — prompt-budget and model-facing guidance.
- [Testing Models](specs/testing-models.md) — deterministic, tmux, and live test policy.
- [Tool Loading And Approval Tiers](specs/tool-loading-and-approval-tiers.md) — future lazy-tool policy.
- [Tools And Modes](specs/tools-and-modes.md) — canonical native tool surface and mode behavior.
- [TUI Architecture](specs/tui-architecture.md) — shell layout, slash commands, and rendering.
- [TUI Hotkeys](specs/tui-hotkeys.md) — keybinding source of truth.
- [Workflows And Recovery](specs/workflows-and-recovery.md) — recovery and workflow behavior.
- [Workspace Trust And Rollback](specs/workspace-trust-and-rollback.md) — trust and checkpoint policy.

## Active Topic Docs

- [Active-Turn Steering Contract](design/active-turn-steering-contract-2026-04-30.md) — queued input now, true steering later.
- [Core Agent Reference Delta](research/core-agent-reference-delta-2026-04-27.md) — Pi/Codex/Claude CLI, command, fork, and goal deltas.
- [Pi Core Loop Review](research/pi-current-core-loop-review-2026-04.md) — Pi loop and UX reference notes.
- [Prompt Budget Baseline](research/prompt-budget-2026-05.md) — measured static prompt size and no-cache decision.
- [Skills Progressive Disclosure](research/skills-progressive-disclosure-sota-2026-04.md) — evidence behind skill loading policy.
- [Tool Surface Research](research/tool-surface-sota-2026-05.md) — search/edit/tool-shape reference evidence.
- [Canto Core Loop Audit](review/canto-core-loop-contract-audit-2026-04-27.md) — Canto contract evidence relevant to Ion.
- [Core Loop Contract](review/core-loop-contract.md) — submit/stream/tool/cancel/error/persist/replay invariants.
- [Core Loop Review Tracker](review/core-loop-review-tracker-2026-04-28.md) — remaining native-loop review evidence.
